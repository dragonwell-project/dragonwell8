...
public final class ManagementSupport {
    ...
    // This allows:
    //
    // 1) openJDK tests to run without commercial features being
    // unlocked, which would be the case if FlightRecorder#getEventTypes would
    // be used.
    //
    // 2 discoverability, so event settings can be exposed without the need to
    // create a new Recording in FlightrecorderMXBean.
    //
    // 3) a graphical JMX client to list all attributes to the user, without
