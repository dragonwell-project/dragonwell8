 ...
 * <p>
 * Flight Recorder can be configured in the following ways. There are
 * <ul>
 ...
 * syntax and the {@link ConfigurationInfo} for configuration related information..
 *
 ...
 * machine where the target VM is running. If destination is a relative path, it
 * is relative to the working directory where the target VM was started.}</td>
...
public interface FlightRecorderMXBean extends PlatformManagedObject {
     ...
     *
     * @return a unique identifier that can be used to start, stop, close and
     *         configure the recording
     *
     * @throws IllegalStateException if Flight Recorder can't be started (for
     ...
     *         caller does not have {@code ManagementPermission("control")}
     *
     * @return a unique identifier that can be used for reading recording data.
     *
     * @see RecordingT
     */
     ...
     *        returning.
     *
     * @return a unique identifier that can be used to start, stop,
    ...
    /**
     * Returns a map that contains the settings for the recording with the specified ID,
     * (for example, the event thresholds)
     * <p>
     * If multiple recordings are running at the same time, more data could be
     * recorded than what is specified in this map.
     * <p>
     * The name in the map is the event name and the setting name separated by
     * {@code "#"} (for example, {@code "com.oracle.jdk.VMInfo#period"}). The value
    ...
    /**
     * Returns the list of currently registered event typess.
     * <p>
     * <b>MBeanServer access</b>:<br>
