...
#include "jfr/recorder/access/jfrEventClass.hpp"
...
static ClassFileStream* create_new_bytes_for_event_klass(const InstanceKlass* ik, const ClassFileParser& parser, TRAPS) {
  ...
  ClassFileStream* orig_stream = const_cast<ClassFileStream*>(parser.clone_stream());
...
static ClassFileStream* create_new_bytes_for_subklass(InstanceKlass* ik, const ClassFileParser& parser, Thread* t) {
  assert(JdkJfrEvent::is_a(ik), "invariant");
...
static instanceKlassHandle create_new_instance_klass(InstanceKlass* ik, ClassFileStream* stream, TRAPS) {
  assert(stream != NULL, "invariant");
  ...
  instanceKlassHandle new_ik = new_parser.parseClassFile(class_name,
                                                         cld,
                                                         pd,
                                                         parsed_name,
                                                         true,
...
static void rewrite_klass_pointer(instanceKlassHandle& ik, instanceKlassHandle new_ik, ClassFileParser& parser, TRAPS) {
  assert(ik() != NULL, "invariant");
  assert(new_ik() != NULL, "invariant");
  assert(new_ik->name() != NULL, "invariant");
...
// target for TRACE_KLASS_CREATION hook
void JfrEventClassTransformer::on_klass_creation(instanceKlassHandle& ik, ClassFileParser& parser, TRAPS) {
  assert(ik() != NULL, "invariant");
  if (JdkJfrEvent::is(ik())) {
    ResourceMark rm(THREAD);
    HandleMark hm(THREAD);
    ClassFileStream* new_stream = create_new_bytes_for_event_klass(InstanceKlass::cast(ik()), parser, THREAD);
    if (new_stream == NULL) {
      log_error(jfr, system)("JfrClassAdapter: unable to create ClassFileStream");
      return;
    }
    assert(new_stream != NULL, "invariant");
    instanceKlassHandle new_ik = create_new_instance_klass(InstanceKlass::cast(ik()), new_stream, THREAD);
    if (new_ik() == NULL) {
  ...
  assert(new_stream != NULL, "invariant");
  instanceKlassHandle new_ik = create_new_instance_klass(ik(), new_stream, THREAD);
  if (new_ik() == NULL) {
    log_error(jfr, system)("JfrClassAdapter: unable to create InstanceKlass");
    return;
  }
