...
class ConstantTypeRootSystem : public JfrConstantSerializer {
 public:
  void write_constants(JfrCheckpointWriter& writer) {
    const u4 nof_root_systems = OldObjectRoot::_number_of_systems;
    writer.write_number_of_constants(nof_root_systems);
...
class ConstantRootType : public JfrConstantSerializer {
 public:
  void write_constants(JfrCheckpointWriter& writer) {
    const u4 nof_root_types = OldObjectRoot::_number_of_types;
    writer.write_number_of_constants(nof_root_types);
...
void ObjectSampleCheckpoint::write(const EdgeStore* edge_store, bool emit_all, Thread* thread) {
  ...
  static bool constant_types_registered = false;
  if (!constant_types_registered) {
    JfrConstantSerializer::register_serializer(CONSTANT_TYPE_OLDOBJECTROOTSYSTEM, false, true, new ConstantTypeRootSystem());
    JfrConstantSerializer::register_serializer(CONSTANT_TYPE_OLDOBJECTROOTTYPE, false, true, new ConstantRootType());
    constant_types_registered = true;
  }
  const ObjectSampler* const object_sampler = LeakProfiler::object_sampler();
  assert(object_sampler != NULL, "invariant");
...
bool WriteObjectSampleStacktrace::process() {
  ...
  writer.write_constant_type(CONSTANT_TYPE_STACKTRACE);
  ...
  writer.write_number_of_constants((u4)count, count_offset);
