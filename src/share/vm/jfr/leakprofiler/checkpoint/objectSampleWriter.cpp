...
class FieldTable : public ResourceObj {
 ...
 private:
  ...
  bool equals(const ObjectSampleFieldInfo* query, uintptr_t hash, const FieldInfoEntry* entry) {
    ...
    assert(const_cast<Symbol*>(stored->_field_name_symbol)->identity_hash() == const_cast<Symbol*>(query->_field_name_symbol)->identity_hash(), "invariant");
...
typedef JfrArtifactWriterHost<SampleWriterImpl, CONSTANT_TYPE_OLDOBJECT> SampleWriter;
...
typedef JfrArtifactWriterHost<ReferenceWriterImpl, CONSTANT_TYPE_REFERENCE> ReferenceWriter;
...
typedef JfrArtifactWriterHost<ArrayWriterImpl, CONSTANT_TYPE_OLDOBJECTARRAY> ArrayWriter;
...
typedef JfrArtifactWriterHost<FieldWriterImpl, CONSTANT_TYPE_OLDOBJECTFIELD> FieldWriter;
...
typedef JfrArtifactWriterHost<RootDescriptionWriterImpl, CONSTANT_TYPE_OLDOBJECTGCROOT> RootDescriptionWriter;
