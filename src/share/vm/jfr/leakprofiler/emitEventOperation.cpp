...
void EmitEventOperation::doit() {
  ...
  _vm_thread_data = _vm_thread->trace_data();
  assert(_vm_thread_data != NULL, "invariant");
  assert(_vm_thread->trace_data()->thread_id() == THREAD_TRACE_ID(_vm_thread), "invariant");
...
int EmitEventOperation::write_events(EdgeStore* edge_store) {
  ...
  const traceid vmthread_id = _vm_thread_data->thread_id();
  assert(_vm_thread_data->thread_id() == THREAD_TRACE_ID(_vm_thread), "invariant");
  ...
  _vm_thread_data->set_thread_id(vmthread_id);
  _vm_thread_data->clear_cached_stack_trace();
  assert(_vm_thread_data->thread_id() == THREAD_TRACE_ID(_vm_thread), "invariant");
...
void EmitEventOperation::write_event(const ObjectSample* sample, EdgeStore* edge_store) {
  ...
  _vm_thread_data->set_cached_stack_trace_id(sample->stack_trace_id());
  assert(sample->has_thread(), "invariant");
  _vm_thread_data->set_thread_id(sample->thread_id());
  e.commit();
}
