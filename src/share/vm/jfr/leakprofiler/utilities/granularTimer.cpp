...
JfrTraceTime GranularTimer::_finish_time_ticks = 0;
JfrTraceTime GranularTimer::_start_time_ticks = 0;
...
void GranularTimer::start(jlong duration_ticks, long granularity) {
  ...
  _start_time_ticks = JfrTraceTime::now();
  const jlong end_time_ticks = _start_time_ticks.value() + duration_ticks;
  _finish_time_ticks = end_time_ticks < 0 ? JfrTraceTime(max_jlong) : JfrTraceTime(end_time_ticks);
  _finished = _finish_time_ticks == _start_time_ticks;
  assert(_finish_time_ticks >= 0, "invariant");
  assert(_finish_time_ticks >= _start_time_ticks, "invariant");
}
void GranularTimer::stop() {
  if (!_finished) {
    _finish_time_ticks = JfrTraceTime::now();
  }
}
const JfrTraceTime& GranularTimer::start_time() {
  return _start_time_ticks;
}

const JfrTraceTime& GranularTimer::end_time() {
...
bool GranularTimer::is_finished() {
  ...
  if (--_counter == 0) {
    ...
    if (JfrTraceTime::now() > _finish_time_ticks) {
      _finished = true;
      _counter = 1;
      return true;
    }
