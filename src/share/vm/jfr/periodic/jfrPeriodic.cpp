...
#include "tracefiles/tracePeriodic.hpp"
...
static int _native_library_callback(const char* name, address base, address top, void *param) {
  ...
  event.set_endtime(*(JfrTraceTime*) param);
  event.commit();
  return 0;
}

TRACE_REQUEST_FUNC(NativeLibrary) {
  JfrTraceTime ts(Tracing::time());
...
TRACE_REQUEST_FUNC(CPUTimeStampCounter) {
  ...
  event.set_fastTimeEnabled(JfrTraceTime::is_ft_enabled());
  event.set_fastTimeAutoEnabled(JfrTraceTime::is_ft_supported());
  event.set_osFrequency(os::elapsed_frequency());
  event.set_fastTimeFrequency(JfrTraceTime::frequency());
  event.commit();
}
...
TRACE_REQUEST_FUNC(SystemProcess) {
  ...
  JfrTraceTime start_time(Tracing::time());
  ...
  JfrTraceTime end_time(Tracing::time());
  if (ret_val == FUNCTIONALITY_NOT_IMPLEMENTED) {
    return;
  }
  if (ret_val == OS_OK) {
...
TRACE_REQUEST_FUNC(InitialSystemProperty) {
  ...
  JfrTraceTime time_stamp(Tracing::time());
  while (p !=  NULL) {
    if (!p->internal()) {
...
TRACE_REQUEST_FUNC(ThreadAllocationStatistics) {
  ...
  JfrTraceTime time_stamp(Tracing::time());
  {
    ...
    while (jt) {
      ...
      thread_ids.append(THREAD_TRACE_ID(jt));
      jt = jt->next();
    }
  }
