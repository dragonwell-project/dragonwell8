...
#include "jfr/recorder/access/jfrMemorySizer.hpp"
...
bool JfrOptionSet::parse_start_flight_recording(const JavaVMOption** option, char* tail) {
  assert(option != NULL, "invariant");
...
bool JfrOptionSet::parse_flight_recorder_options(const JavaVMOption** option, char* tail) {
  assert(option != NULL, "invariant");
  assert(tail != NULL, "invariant");
  assert((*option)->optionString != NULL, "invariant");
