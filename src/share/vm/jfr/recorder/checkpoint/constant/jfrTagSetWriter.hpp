...
#ifndef SHARE_VM_JFR_CHECKPOINT_CONSTANT_JFRTAGSETWRITER_HPP
#define SHARE_VM_JFR_CHECKPOINT_CONSTANT_JFRTAGSETWRITER_HPP
...
template <typename WriterImpl, u4 CONSTANT_TYPE_ID>
class JfrArtifactWriterHost : public StackObj {
 ...
 public:
  JfrArtifactWriterHost(JfrCheckpointWriter* writer,
    ...
    if (!_skip_header) {
      _writer->write_constant_type((JfrConstantTypeId)CONSTANT_TYPE_ID);
  ...
  ~JfrArtifactWriterHost() {
    ...
    if (!_skip_header) {
      _writer->write_number_of_constants(_count, _count_offset);
    }
  }
...
#endif // SHARE_VM_JFR_CHECKPOINT_CONSTANT_JFRTAGSETWRITER_HPP
