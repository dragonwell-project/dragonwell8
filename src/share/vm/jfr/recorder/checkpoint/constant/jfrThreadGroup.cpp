...
JfrThreadGroup::new_entry(JfrThreadGroupPointers& ptrs) {
  const char* str = NULL;
  typeArrayOop name = java_lang_ThreadGroup::name(ptrs.thread_group_oop());
  if (name != NULL) {
    str = UNICODE::as_utf8((jchar*) name->base(T_CHAR), name->length());
  }
  JfrThreadGroupEntry* const tge = new JfrThreadGroupEntry(str, ptrs);
  add_entry(tge);
  return tge;
}
...
void JfrThreadGroup::write_selective_thread_group(JfrCheckpointWriter* writer, traceid thread_group_id) const {
  ...
  writer->write_constant_type(CONSTANT_TYPE_THREADGROUP);
  ...
  writer->write_number_of_constants(number_of_entries_written, count_offset);
}

// Write out JfrThreadGroup instance and then delete it
void JfrThreadGroup::write(JfrCheckpointWriter& writer) {
  ThreadGroupExclusiveAccess lock;
  JfrThreadGroup* tg_instance = instance();
...
void JfrThreadGroup::write(JfrCheckpointWriter* writer, traceid thread_group_id) {
  assert(writer != NULL, "invariant");
  ThreadGroupExclusiveAccess lock;
