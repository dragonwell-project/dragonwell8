...
static void write_checkpoint_header(u1* pos, jlong size, jlong time, bool flushpoint, juint type_count) {
  ...
  be_writer.write(JfrTraceTime::now() - time);
...
void JfrCheckpointWriter::write_constant_type(JfrConstantTypeId constant_type_id) {
  assert(constant_type_id < CONSTANT_TYPES_END, "invariant");
  write<u8>(constant_type_id);
  increment();
}
...
void JfrCheckpointWriter::write_number_of_constants(u4 nof_entries) {
  write<u4>((u4)nof_entries);
}

void JfrCheckpointWriter::write_number_of_constants(u4 nof_entries, jlong offset) {
