...
class JfrEventSetting : AllStatic {
 private:
  ...
  static jfrNativeEventSetting& setting(TraceEventId event_id);

 public:
  static void set_enabled(jlong event_id, bool enabled);
  static bool is_enabled(TraceEventId event_id);
  static void set_stacktrace(jlong event_id, bool enabled);
  static bool has_stacktrace(TraceEventId event_id);
