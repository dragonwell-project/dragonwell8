...
inline jfrNativeEventSetting& JfrEventSetting::setting(TraceEventId event_id) {
  return _jvm_event_settings.bits[event_id];
}

inline bool JfrEventSetting::is_enabled(TraceEventId event_id) {
  return 0 != setting(event_id).enabled;
}

inline bool JfrEventSetting::has_stacktrace(TraceEventId event_id) {
  return 0 != setting(event_id).stacktrace;
}

inline jlong JfrEventSetting::threshold(TraceEventId event_id) {
  return setting(event_id).threshold_ticks;
}

inline jlong JfrEventSetting::cutoff(TraceEventId event_id) {
