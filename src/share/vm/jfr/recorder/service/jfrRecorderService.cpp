...
static bool try_set(void* const data, void** dest, bool clear) {
  ...
  const void* const current = OrderAccess::load_ptr_acquire((void*)dest);
  if (current != NULL) {
    if (current != data) {
      // already set
      return false;
  ...
  return Atomic::cmpxchg_ptr(clear ? NULL : data, dest, const_cast<void*>(current)) == current;
}
...
static intptr_t write_checkpoint_event_prologue(JfrChunkWriter& cw, u8 constant_id) {
  ...
  cw.write(JfrTraceTime::now().value());
  ...
  cw.write<u4>((u4)1); // nof constant types in this checkpoint
  cw.write<u8>(constant_id);
...
class WriteCheckpointEvent : public StackObj {
 private:
  JfrChunkWriter& _cw;
  u8 _constant_type_id;
  ContentFunctor& _content_functor;
 public:
  WriteCheckpointEvent(JfrChunkWriter& cw, u8 constant_type_id, ContentFunctor& functor) :
    _cw(cw),
    _constant_type_id(constant_type_id),
  ...
  bool process() {
    ...
    const intptr_t num_elements_offset = write_checkpoint_event_prologue(_cw, _constant_type_id);
    // invocation
...
static void write_stacktrace_checkpoint(JfrStackTraceRepository& stack_trace_repo, JfrChunkWriter& chunkwriter, bool clear) {
  ...
  WriteStackTraceCheckpoint write_stack_trace_checkpoint(chunkwriter, CONSTANT_TYPE_STACKTRACE, write_stacktrace_repo);
...
static void write_stringpool_checkpoint(JfrStringPool& string_pool, JfrChunkWriter& chunkwriter) {
  ...
  WriteStringPoolCheckpoint write_string_pool_checkpoint(chunkwriter, CONSTANT_TYPE_STRING, write_string_pool);
...
void JfrRecorderService::pre_safepoint_write() {
  MutexLockerEx stream_lock(JfrStream_lock, Mutex::_no_safepoint_check_flag);
  assert(_chunkwriter.is_valid(), "invariant");
  _checkpoint_manager.write_constant_types();
  _checkpoint_manager.write_epoch_transition_list();
...
//         write string pool ->
//           write safepoint dependent constant types ->
//             write storage ->
//                 shift_epoch ->
//                   update time ->
...
void JfrRecorderService::safepoint_write() {
  ...
  write_stringpool_checkpoint(_string_pool, _chunkwriter);
  _checkpoint_manager.write_safepoint_constant_types();
...
void JfrRecorderService::post_safepoint_write() {
  ...
  // Constant tagging is epoch relative which entails we are able to write out the
  // already tagged artifacts for the previous epoch. We can accomplish this concurrently
  // with threads now tagging artifacts in relation to the new, now updated, epoch and remain outside of a safepoint.
  _checkpoint_manager.write_constant_tag_set();
  MutexLockerEx stream_lock(JfrStream_lock, Mutex::_no_safepoint_check_flag);
...
void JfrRecorderService::finalize_current_chunk_on_vm_error() {
  ...
  // Optimistically write tagged constant artifacts.
  _checkpoint_manager.shift_epoch();
  _checkpoint_manager.write_constant_tag_set();
  // update time
  _chunkwriter.time_stamp_chunk_now();
