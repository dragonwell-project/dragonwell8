...
JfrStackTraceRepository::JfrStackTraceRepository() : _next_id(0) {
  memset(_table, 0, sizeof(_table));
}
class JfrFrameConstantType : public JfrConstantSerializer {
 public:
  void write_constants(JfrCheckpointWriter& writer) {
    writer.write_number_of_constants(JfrStackFrame::NUM_FRAME_TYPES);
...
bool JfrStackTraceRepository::initialize() {
  return JfrConstantSerializer::register_serializer(CONSTANT_TYPE_FRAMETYPE, false, true, new JfrFrameConstantType());
}
...
traceid JfrStackTraceRepository::record(Thread* thread, int skip /* 0 */) {
  ...
  if (trace_data->has_cached_stack_trace()) {
    return trace_data->cached_stack_trace_id();
  }
  if (!thread->is_Java_thread() || thread->is_hidden_from_external_view()) {
    return 0;
  }
  JfrStackFrame* frames = trace_data->stackframes();
  if (frames == NULL) {
    // pending oom
    return 0;
  }
...
traceid JfrStackTraceRepository::record(Thread* thread, int skip, unsigned int* hash) {
  ...
  if (trace_data->has_cached_stack_trace()) {
    *hash = trace_data->cached_stack_trace_hash();
    return trace_data->cached_stack_trace_id();
  }
  if (!thread->is_Java_thread() || thread->is_hidden_from_external_view()) {
    return 0;
...
void JfrStackTraceRepository::write_metadata(JfrCheckpointWriter& writer) {
  JfrFrameConstantType fct;
  writer.write_constant_type(CONSTANT_TYPE_FRAMETYPE);
  fct.write_constants(writer);
}
