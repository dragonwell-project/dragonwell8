...
template <typename Mspace>
class MutexedReleaseOp : public ReleaseOp<Mspace> {
 private:
  Thread* _thread;
 public:
  typedef typename Mspace::Type Type;
  MutexedReleaseOp(Mspace* mspace, Thread* thread) : ReleaseOp<Mspace>(mspace), _thread(thread) {}
  bool process(Type* t);
  size_t processed() const { return ReleaseOp<Mspace>::processed(); }
};

template <typename Mspace>
class ThreadLocalReleaseOp : public StackObj {
 private:
  Mspace* _mspace;
 public:
  typedef typename Mspace::Type Type;
  ThreadLocalReleaseOp(Mspace* mspace) : _mspace(mspace) {}
  bool process(Type* t);
  size_t processed() const { return 0; }
};
