...
template <typename T, template <typename> class RetrievalType, typename Callback>
inline void JfrMemorySpace<T, RetrievalType, Callback>::release(T* t) {
...
template <typename Mspace>
inline void mspace_release(typename Mspace::Type* t, Mspace* mspace) {
  assert(t != NULL, "invariant");
...
template <typename Mspace>
inline void mspace_release_critical(typename Mspace::Type* t, Mspace* mspace) {
  MspaceLock<Mspace> lock(mspace);
  mspace_release(t, mspace);
}
...
template <typename Mspace>
inline bool MutexedReleaseOp<Mspace>::process(typename Mspace::Type* t) {
  assert(t != NULL, "invariant");
  if (t->retired() || t->try_acquire(_thread)) {
    return ReleaseOp<Mspace>::process(t); // mutexed access
  }
  return true;
}

template <typename Mspace>
inline bool ThreadLocalReleaseOp<Mspace>::process(typename Mspace::Type* t) {
  assert(t != NULL, "invariant");
