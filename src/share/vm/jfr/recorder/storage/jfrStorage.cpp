...
static JfrStorageMspace* create_mspace(size_t buffer_size, size_t limit, size_t cache_count, JfrStorage* storage_instance) {
  JfrStorageMspace* mspace = new JfrStorageMspace(buffer_size, limit, cache_count, storage_instance);
  if (mspace != NULL) {
    mspace->initialize();
  }
  return mspace;
...
bool JfrStorage::initialize() {
  ...
  _global_mspace = create_mspace(global_buffer_size, memory_size, num_global_buffers, this);
  if (_global_mspace == NULL) {
    return false;
  }
  _thread_local_mspace = create_mspace(thread_buffer_size, unlimited_mspace_size, thread_local_cache_count, this);
  if (_thread_local_mspace == NULL) {
    return false;
  }
  _transient_mspace = create_mspace(thread_buffer_size * transient_buffer_size_multiplier, unlimited_mspace_size, 0, this);
  if (_transient_mspace == NULL) {
    return false;
  }
  _age_mspace = new JfrStorageAgeMspace(0 /* no extra size except header */, unlimited_mspace_size, num_global_buffers, this);

  if (_age_mspace == NULL || !_age_mspace->initialize()) {
    return false;
  }
...
static void write_data_loss_event(size_t unflushed_size, Thread* thread) {
  const u8 total_data_loss = thread->trace_data()->add_data_lost(unflushed_size);
  if (EventDataLoss::is_enabled()) {
    EventDataLoss dataloss;
    dataloss.set_amount(unflushed_size);
    dataloss.set_total(total_data_loss);
    dataloss.commit();
  }
}
...
static void write_data_loss(BufferPtr buffer, Thread* thread) {
  ...
  write_data_loss_event(unflushed_size, thread);
}
...
void JfrStorage::discard_oldest(Thread* thread) {
  if (JfrBuffer_lock->try_lock()) {
    ...
    while (true) {
      ...
      if (buffer->transient()) {
        mspace_release(buffer, _transient_mspace);
        mspace_release(oldest_age_node, _age_mspace);
        continue;
      } else {
        mspace_release(oldest_age_node, _age_mspace);
        buffer->reinitialize();
...
static void assert_flush_precondition(ConstBufferPtr cur, size_t used, bool native, const Thread* t) {
  ...
  assert(native ? t->trace_data()->native_buffer() == cur : t->trace_data()->java_buffer() == cur, "invariant");
}
...
static void assert_flush_regular_precondition(ConstBufferPtr cur, const u1* const cur_pos, size_t used, size_t req, const Thread* t) {
  assert(t != NULL, "invariant");
  assert(t->trace_data()->shelved_buffer() == NULL, "invariant");
  assert(cur != NULL, "invariant");
...
static void assert_flush_large_precondition(ConstBufferPtr cur, const u1* const cur_pos, size_t used, size_t req, bool native, Thread* t) {
  ...
  assert(t->trace_data()->shelved_buffer() != NULL, "invariant");
  assert(req >= used, "invariant");
  assert(cur != t->trace_data()->shelved_buffer(), "invariant");
}
#endif // ASSERT
...
BufferPtr JfrStorage::flush(BufferPtr cur, size_t used, size_t req, bool native, Thread* t) {
  ...
  if (cur->lease()) {
    return instance().flush_large(cur, cur_pos, used, req, native, t);
  }
  // "regular" == !lease
  return instance().flush_regular(cur, cur_pos, used, req, native, t);
}
...
BufferPtr JfrStorage::flush_regular(BufferPtr cur, const u1* const cur_pos, size_t used, size_t req, bool native, Thread* t) {
  ...
  t->trace_data()->shelve_buffer(cur);
  return provision_large(cur, cur_pos, used, req, native, t);
}

static BufferPtr store_buffer_to_thread_local(BufferPtr buffer, JfrThreadData* trace_data, bool native) {
  assert(buffer != NULL, "invariant");
  if (native) {
    trace_data->set_native_buffer(buffer);
  } else {
   trace_data->set_java_buffer(buffer);
  }
  return buffer;
}
...
static BufferPtr restore_shelved_buffer(bool native, Thread* t) {
  ...
  BufferPtr shelved = trace_data->shelved_buffer();
  assert(shelved != NULL, "invariant");
  t->trace_data()->shelve_buffer(NULL);
  // restore shelved buffer back as primary
  return store_buffer_to_thread_local(shelved, trace_data, native);
}
...
BufferPtr JfrStorage::flush_large(BufferPtr cur, const u1* const cur_pos, size_t used, size_t req, bool native, Thread* t) {
  ...
  BufferPtr shelved = t->trace_data()->shelved_buffer();
  assert(shelved != NULL, "invariant");
...
BufferPtr JfrStorage::provision_large(BufferPtr cur, const u1* const cur_pos, size_t used, size_t req, bool native, Thread* t) {
  ...
  return store_buffer_to_thread_local(buffer, t->trace_data(), native);
}
...
typedef ThreadLocalReleaseOp<JfrStorageMspace> ThreadLocalRelease;
typedef CompositeOperation<ConcurrentWriteOperation, ThreadLocalRelease> ThreadLocalWriteOperation;

size_t JfrStorage::write() {
  const size_t full_size_processed = write_full();
  WriteOperation wo(_chunkwriter);
  ConcurrentWriteOperation cwo(wo);
  ThreadLocalRelease tlr(_thread_local_mspace);
  ThreadLocalWriteOperation tlwo(&cwo, &tlr);
...
typedef CompositeOperation<DiscardOperation, ThreadLocalRelease> ThreadLocalDiscardOperation;
...
size_t JfrStorage::clear() {
  ...
  ThreadLocalRelease tlr(_thread_local_mspace);
  ThreadLocalDiscardOperation tldo(&discarder, &tlr);
...
size_t JfrStorage::write_full() {
  ...
  ReleaseOperation ro(_transient_mspace);
 ...
 public:
  ...
  bool process(Type* t) {
    if (t->retired()) {
      ...
      t->clear_retired();
      mspace_release_critical(t, _mspace);
    }
    return true;
  }
...
size_t JfrStorage::scavenge() {
  if (control().dead_count() > 0) {
    Scavenger<JfrStorageMspace> scavenger(control(), _thread_local_mspace);
    process_full_list(scavenger, _thread_local_mspace);
    scavenge_log(scavenger.processed(), scavenger.amount(), control().dead_count());
