...
void JfrTimeConverter::initialize() {
  if (!initialized) {
    ...
    if (JfrTraceTime::is_ft_enabled()) {
      nanos_to_ft_counter_factor = (double)JfrTraceTime::frequency() / NANOS_PER_SEC;
...
double JfrTimeConverter::counter_to_nano_multiplier(bool is_os_time) {
  ...
  return JfrTraceTime::is_ft_enabled() && !is_os_time ? ft_counter_to_nanos_factor : os_counter_to_nanos_factor;
}
...
double JfrTimeConverter::nano_to_counter_multiplier(bool is_os_time) {
  ...
  return JfrTraceTime::is_ft_enabled() && !is_os_time ? nanos_to_ft_counter_factor : nanos_to_os_counter_factor;
}
