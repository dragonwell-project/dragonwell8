...
jboolean JfrJavaEventWriter::flush(jobject writer, jint used, jint requested, JavaThread* jt) {
  ...
  JfrBuffer* const current = jt->trace_data()->java_buffer();
...
void JfrJavaEventWriter::notify(JavaThread* jt) {
  ...
  assert(SafepointSynchronize::is_at_safepoint(), "invariant");
  if (jt->trace_data()->has_java_event_writer()) {
    oop buffer_writer = JNIHandles::resolve_non_null(jt->trace_data()->java_event_writer());
...
static jobject create_new_event_writer(JfrBuffer* buffer, TRAPS) {
  ...
  args.push_long((jlong)THREAD_TRACE_ID(THREAD));
...
jobject JfrJavaEventWriter::event_writer(Thread* t) {
  ...
  assert(trace_data->shelved_buffer() == NULL, "invariant");
  return trace_data->java_event_writer();
}
...
jobject JfrJavaEventWriter::new_event_writer(TRAPS) {
  ...
  assert(!trace_data->has_java_buffer(), "invariant");
  JfrBuffer* const buffer = trace_data->java_buffer();
  if (buffer == NULL) {
  ...
  trace_data->set_java_event_writer(java_event_writer);
  assert(trace_data->has_java_event_writer(), "invariant");
