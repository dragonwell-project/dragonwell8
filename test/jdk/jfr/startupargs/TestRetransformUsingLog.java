...
import jdk.testlibrary.jfr.StartupHelper;
...
public class TestRetransformUsingLog {
    ...
    private static final String FILE_READ_FORCED_CLASS_LOAD = "Adding forced instrumentation for event type com.oracle.jdk.FileRead during initial class load";
    ...
    private static void startApp(boolean recording, boolean retransform, Consumer<OutputAnalyzer> verifier) throws Exception {
        ...
        if (recording) {
            args.add("-XX:StartFlightRecording=name=myrec,settings=profile");
        }
